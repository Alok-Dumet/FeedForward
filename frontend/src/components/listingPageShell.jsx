//We will render listing page headings, filters, and items while callers decide how each item becomes a card
export default function ListingPageShell({ sectionLabel, description, items, filters = [], filtersLabel = 'Filters:', activeFilters = [], onFilterChange, extraControls = null, renderItem }) {
  return (
        ...
        <section className="rounded-[2rem] border border-white/70 bg-slate-950 px-8 py-10 text-white shadow-2xl">
          <div className="flex flex-wrap items-start justify-between gap-6">
            <div className="max-w-3xl">
              {sectionLabel ? <p className="text-sm font-medium tracking-[0.2em] text-emerald-400 uppercase">{sectionLabel}</p> : null}
              <h1 className="mt-4 max-w-3xl text-3xl font-extrabold text-white sm:text-4xl">{description}</h1>
            </div>
          </div>
        </section>

        <section className="surface-glass-panel px-6 py-5">
